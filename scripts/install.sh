...
eval "$("$SCRIPT_DIR/compute-install-counts.sh")"
...
copy_skill_dirs
echo "  Skills: $(find "$CODEX_ROOT/skills" -name 'SKILL.md' 2>/dev/null | wc -l | tr -d ' ') installed (expected ${SKILL_COUNT})"
...
echo ""
echo "Re-run the same bootstrap or install command later to refresh to the latest published main branch."
