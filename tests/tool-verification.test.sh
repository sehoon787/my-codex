...
grep -q '^  codeburn shared dashboard: http://127.0.0.1:4747/ (started or reused during installation)$' <<<"$success_output"
grep -q '^  Headroom proxy stats: http://127.0.0.1:8787/stats (empty until traffic is explicitly routed)$' <<<"$success_output"
...
grep -q '^  codeburn shared dashboard: http://127.0.0.1:4747/ (started or reused during installation)$' <<<"$bad_temp_output"
grep -q '^  Headroom proxy stats: http://127.0.0.1:8787/stats (empty until traffic is explicitly routed)$' <<<"$bad_temp_output"
