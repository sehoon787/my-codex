...
if [ "$VAULT_ONLY" = "1" ]; then
  ...
  (
    ...
    if [ -n "$GIT_BASH" ]; then
      ...
      printf '{"prompt":"Summarize the work done so far"}' | HOME="$TEST_HOME" node "$TEST_HOME/.codex/hooks/session-sync.js" prompt > "$TMP_ROOT/hook-prompt-2.out"
